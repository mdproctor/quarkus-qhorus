...
class EphemeralSemanticTest {
    ...
    void ephemeralMessageDeliveredOnFirstRead() {
        tools.createChannel("eph-1", "EPHEMERAL channel", "EPHEMERAL", null);
        tools.sendMessage("eph-1", "alice", "status", "routing hint", null, null);

        CheckResult result = tools.checkMessages("eph-1", 0L, 10, null);
    ...
    void ephemeralSecondReadReturnsEmpty() {
        tools.createChannel("eph-2", "EPHEMERAL channel", "EPHEMERAL", null);
        tools.sendMessage("eph-2", "alice", "status", "transient context", null, null);

        tools.checkMessages("eph-2", 0L, 10, null); // First read — consumes

        CheckResult second = tools.checkMessages("eph-2", 0L, 10, null);
    ...
    void ephemeralNewWriteAfterReadIsDeliveredOnNextRead() {
        tools.createChannel("eph-3", "EPHEMERAL channel", "EPHEMERAL", null);
        tools.sendMessage("eph-3", "alice", "status", "first hint", null, null);
        tools.checkMessages("eph-3", 0L, 10, null); // Consume first

        tools.sendMessage("eph-3", "alice", "status", "second hint", null, null);
        CheckResult result = tools.checkMessages("eph-3", 0L, 10, null);
    ...
    void ephemeralMultipleMessagesAllConsumedOnFirstRead() {
        tools.createChannel("eph-4", "EPHEMERAL channel", "EPHEMERAL", null);
        tools.sendMessage("eph-4", "alice", "status", "hint-1", null, null);
        tools.sendMessage("eph-4", "bob", "status", "hint-2", null, null);
        tools.sendMessage("eph-4", "carol", "status", "hint-3", null, null);

        CheckResult first = tools.checkMessages("eph-4", 0L, 10, null);
        assertEquals(3, first.messages().size());

        CheckResult second = tools.checkMessages("eph-4", 0L, 10, null);
        assertTrue(second.messages().isEmpty());
    }
    ...
    void appendChannelUnaffectedByEphemeralLogic() {
        ...
        tools.checkMessages("append-eph", 0L, 10, null);

        // APPEND messages survive reads
        CheckResult second = tools.checkMessages("append-eph", 0L, 10, null);
        assertEquals(1, second.messages().size(),
