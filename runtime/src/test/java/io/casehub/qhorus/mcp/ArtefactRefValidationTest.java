...
class ArtefactRefValidationTest {
    ...
    void sendMessageWithValidArtefactRefSucceeds() {
        ...
        assertDoesNotThrow(() -> tools.sendMessage("arv-ch-1", "alice", "status", "with valid ref",
                null, null, List.of(artefact.artefactId().toString())));
    }
    ...
    void sendMessageWithNullArtefactRefsSkipsValidation() {
        tools.createChannel("arv-ch-4", "Test", null, null);

        // null refs should always succeed — no validation needed
        assertDoesNotThrow(() -> tools.sendMessage("arv-ch-4", "alice", "status", "no refs",
                null, null, null));
    }
    ...
    void sendMessageWithEmptyArtefactRefsListSkipsValidation() {
        tools.createChannel("arv-ch-5", "Test", null, null);

        assertDoesNotThrow(() -> tools.sendMessage("arv-ch-5", "alice", "status", "empty refs",
                null, null, List.of()));
    }
    ...
    void sendMessageWithIncompleteArtefactRefIsAllowed() {
        ...
        assertDoesNotThrow(() -> tools.sendMessage("arv-ch-6", "alice", "status", "ref to incomplete",
                null, null, List.of(incomplete.artefactId().toString())),
